...
class MetricLogger:
    ...
    def __init__(self, metric_list: List[Tuple[str, bool]]):
        ...
        metric_list = [(self.CUM_TIME, True), (self.ITR_TIME, False), *metric_list]
    ...
    def end_itr(self, itr_time: Optional[float] = None):
        ...
        remaining_metrics = (
            set(self._metrics.keys())
            - self._added_this_itr
            - {self.ITR_TIME, self.CUM_TIME}
        )
        if len(remaining_metrics) > 0:
            warnings.warn(
                "The following metrics were not added this " f"itr: {remaining_metrics}"
            )
        for metric in remaining_metrics:
            self._metrics[metric]["data"].append(0)

        # Handle time.
        itr_time = (
            itr_time if itr_time is not None else time.time() - self._itr_start_time
        )
    ...
    def add(self, name: str, value: Union[float, int], logger: logging.Logger = None):
    ...
    def add_post(
        self, name: str, values: Union[Sequence[float], Sequence[int]], use_zero: bool
    ):
        ...
        if len(values) != expected_length:
            raise ValueError(
                f"values should be length {expected_length} but "
    ...
    def get_single(self, name: str) -> Dict:
        ...
        return {
            "x": list(
                range(int(not self._metrics[name]["use_zero"]), self._total_itrs + 1)
            ),
            "y": self._metrics[name]["data"],
        }
