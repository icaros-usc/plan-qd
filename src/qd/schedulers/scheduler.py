...
Adapted from pyribs 0.7.1 to allow success masks.
"""
...
class Scheduler:
    ...
    def __init__(self, archive, emitters, *, result_archive=None, add_mode="batch"):
        ...
        if add_mode not in ["single", "batch"]:
            raise ValueError(
                "add_mode must either be 'batch' or 'single', but "
                f"it was '{add_mode}'"
            )
        ...
        if result_archive is not None and set(archive.field_list) != set(
            ...
            raise ValueError(
                "`archive` and `result_archive` should have the "
    ...
    def _add_to_archives(self, data, success_mask):
        ...
        if self._add_mode == "batch":
            ...
            masked_add_info = defaultdict(list)
            ...
            for i in range(len(self._cur_solutions)):
                ...
                for name, val in single_info.items():
                    masked_add_info[name].append(val)
            ...
            for name, arr in masked_add_info.items():
                masked_add_info[name] = np.asarray(arr)
    ...
    def tell_dqd(self, objective, measures, jacobian, success_mask=None, **fields):
        ...
        for emitter, n in zip(self._emitters, self._num_emitted):
            ...
            emitter.tell_dqd(
                **{name: arr[pos:end][mask] for name, arr in data.items()},
                jacobian=jacobian[pos:end][mask],
